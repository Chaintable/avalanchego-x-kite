...
func NewBlockChain(
	...
	if pipelineTracer, ok := vmConfig.Tracer.(*tracer.PipelineTracer); ok && pipelineTracer != nil {
		bc.hooks = tracing.BuildHooks(pipelineTracer)
	}
...
func (bc *BlockChain) writeBlockAndSetHead(block *types.Block, parentRoot common.Hash, receipts []*types.Receipt, logs []*types.Log, state *state.StateDB) error {
	...
	if bc.newTip(block) {
		...
		// A backup still produces objects, but only the leader publishes notifications.
		if bc.hooks == nil || tracer.NodeXPusher == nil || leader.GlobalManager == nil || !leader.GlobalManager.IsLeader() {
			return nil
		}
...
func (bc *BlockChain) reprocessBlock(parent *types.Block, current *types.Block) (common.Hash, error) {
	...
	// Process previously stored block with the same writer hooks as live blocks.
...
func (bc *BlockChain) commitWithSnap(
	...
	// libevm only calls OnCommit for changed roots. Empty-state blocks still
	// need headers, blockfiles and validation objects, including on backups.
	if root == parentRoot && bc.hooks != nil && bc.hooks.OnCommit != nil {
